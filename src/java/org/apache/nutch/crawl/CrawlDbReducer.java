...
public class CrawlDbReducer implements Reducer<Text, CrawlDatum, Text, CrawlDatum> {
  ...
  public void reduce(Text key, Iterator<CrawlDatum> values,
    ...
    //FIXME added to update the score
    /***
     * @author swapnil
     * **/
		System.out.println(" In crawl DB Reducer: " + fetch.getScore() + "\t"
				+ CrawlDatum.getStatusName(fetch.getStatus()) + "\t" + key);
		URLDetails details = null;
		if (Crawl.urlSet.size() > 0) {
			details = new URLDetails(key.toString());
			if (Crawl.urlSet.indexOf(details) >= Crawl.TOPN
					|| Crawl.urlSet.indexOf(details) == -1) {
				System.out.println("No information found");
				details = null;
				// return;
			} else {
				details = Crawl.urlSet.get(Crawl.urlSet.indexOf(details));
				// update crawlDatum with new Score
				fetch.setScore(details.getFinalScore());
				System.out.println("Settoiing score of "
						+ details.getFinalScore() + " to url :" + key);
			}
		}
    /**
     * changes end
    ...
    case CrawlDatum.STATUS_LINKED:                // it was link
      if (oldSet) {                          // if old exists
        result.set(old);   
        System.out.println("Using oldddddddddddddddddd");// use it
      } else {
    ...
    case CrawlDatum.STATUS_FETCH_NOTMODIFIED:     // successful fetch, notmodified
      ...
      if (modified == FetchSchedule.STATUS_NOTMODIFIED) {
        ...
        case CrawlDatum.STATUS_FETCH_SUCCESS:
          ...
          if(details!=null) {
        	  Crawl.urlSet.remove(details);
        	  System.out.println("removing" + details.getUrl());
    ...
    try {
    	...
    	//TODO
    	//TODO ask arjun why?
//    	URLDetails.addParentScoresToStatic(score);
